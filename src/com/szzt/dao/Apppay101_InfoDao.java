...
public class Apppay101_InfoDao extends AbstractDao<Apppay101_Info>
{
	public static final String TABLE = "APPPAY_101";

	public Apppay101_InfoDao()
	{
	}

	public Apppay101_InfoDao(String where, String table)
	{
		super(where, table);
	}
	...
	public Apppay101_Info[] select(String sql) throws Exception
	{
		...
		Apppay101_Info[] infos = new Apppay101_Info[this.getPageSize()];
		...
		for (int i = this.getStart(); i < this.getEnd(); i++)
		{
			if (this.getTotalCount() == 0)
				break;
			rs.absolute(i + 1);
			Apppay101_Info info = wrapToModel(rs);
			// ---------------
			infos[j++] = info;
		}

		super.debug("查询出Apppay101_info信息总数为：" + this.getTotalCount());
		rs.close();

		return infos;
	}

	private Apppay101_Info wrapToModel(ResultSet rs) throws SQLException
	{
	...
	@Override
	public int insert(Apppay101_Info appay) throws Exception
	{
		System.out.println("===========>term1:" + appay.getTERM_ID1());
		String sql = "insert into APPPAY_101 ";
		String fields = "APPPAY_ID";
		String values = "APPPAY_ID_ID.nextval";

		if (appay.getCOMMERCIAL_ID() > 0)
		{
		...
		if (appay.getAPPTYPE() != null && !"".equals(appay.getAPPTYPE()))
		{
			fields += ",APPTYPE";
		...
		if (appay.getACCOUNT_NO() != null && !"".equals(appay.getACCOUNT_NO()))
		{
			fields += ",ACCOUNT_NO";
			values += "," + appay.getACCOUNT_NO() + "";
		}
		if (appay.getACCOUNT_BANK() != null
				&& !"".equals(appay.getACCOUNT_BANK()))
		{
			fields += ",ACCOUNT_BANK";
		...
		if (appay.getEC_NAME() != null && !"".equals(appay.getEC_NAME()))
		{
			fields += ",EC_NAME";
			values += "," + appay.getEC_NAME() + "";
		}

		if (appay.getTERM_ID1() != null && !"".equals(appay.getTERM_ID1()))
		{
			fields += ",TERM_ID1";
			values += "," + appay.getTERM_ID1() + "";
		}
		if (appay.getTERM_ID2() != null && !"".equals(appay.getTERM_ID2()))
		{
			fields += ",TERM_ID2";
			values += "," + appay.getTERM_ID2() + "";
		}
		if (appay.getACCOUNT_BANK_NAME() != null
				&& !"".equals(appay.getACCOUNT_BANK_NAME()))
		{
			fields += ",ACCOUNT_BANK_NAME";
			values += "," + appay.getACCOUNT_BANK_NAME() + "";
		}
		// 插入时不需要审批人员
		// if (appay.getAPPR_MAN() != null && !"".equals(appay.getAPPR_MAN()))
		// {
		// fields += ",APPR_MAN";
		...
		// int id = super.loadCurrentId(dba, "APPPAY_101", "APPPAY_ID");
		int id = super.loadAppCurrId(dba, "APPPAY_101", "APPPAY_ID",
	...
	public void update(Apppay101_Info data) throws Exception
	{
		...
		case AppayStatus.ALLOW:
			...
			Apppay101_Info info = this.findById(data.getAPPPAY_ID());
			// 账户账号
		...
		sql.append(" where APPPAY_ID=").append(data.getAPPPAY_ID());
		super.executeUpdate(sql.toString());
		super.debug("Apppay101--update-->" + sql.toString());
	}
	...
	public void delete(int id) throws Exception
	{
		...
		if (super.loadAppayStatus(TABLE, id) < 2)
		{
			...
			String delApp01 = "delete from APPPAY_101 where APPPAY_ID=" + id;
			...
			try
			{
				...
				super.debug("------->Apppay101删除<---------");
				super.debug("删除中间表个数--》" + temp);
				super.debug("------->Apppay101删除成功<---------");
			} catch (Exception e)
			{
				if (null != dba)
					dba.rollback();
	...
	// 判断终端处于的状态
	private int checkStatus(int infoId) throws Exception
	{
		Apppay101_Info[] old = this.select("select * from " + TABLE
				+ " where APPPAY_ID=" + infoId);
		if (null != old[0])
		{
			return old[0].getAPPR_STATUS();
		}
		return -1;
	}
	...
	public void operate(Apppay101_Info info) throws Exception
	{
		...
		sql.append(" APPR_DATE=").append("sysdate");
		...
		// super.wrapUpdateSql("APPR_REMARK",
