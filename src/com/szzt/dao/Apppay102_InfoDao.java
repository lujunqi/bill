...
import com.dodou.table.Apppay102_Info;
import com.szzt.exception.DeleteException;
import com.szzt.exception.ForbidUpdateException;

//APPPAY_106_ID_SEQ
public class Apppay102_InfoDao extends AbstractDao<Apppay102_Info>
{
	public static final String TABLE = "APPPAY_102";

	public Apppay102_InfoDao()
	{
	}

	public Apppay102_InfoDao(String where, String table)
	{
		super(where, table);
	}

	@Override
	public int insert(Apppay102_Info appay) throws Exception
	{

		String sql = "insert into APPPAY_102 ";
		String fields = "APPPAY_ID";
		String values = "APPPAY_ID_102_ID.nextval";
		...
		if (appay.getORDER_NO() != null)
		{
			fields += ",ORDER_NO";
			values += ",'" + appay.getORDER_NO() + "'";
		}
		...
		// int id = super.loadCurrentId(dba, "APPPAY_102", "APPPAY_ID");
		int id = super.loadAppCurrId(dba, "APPPAY_102", "APPPAY_ID",
				appay.getCOMMERCIAL_ID());
		return id;
	}

	@Override
	public Apppay102_Info[] select(String sql) throws Exception
	{
		if (null == sql)
		{
			return this.select();
		}
		Apppay102_Info[] infos = new Apppay102_Info[this.getPageSize()];
		...
		for (int i = this.getStart(); i < this.getEnd(); i++)
		{
			if (this.getTotalCount() == 0)
				break;
			rs.absolute(i + 1);
			Apppay102_Info info = wrapToModel(rs);
		...
		super.debug("查询出Apppay102_info信息总数为：" + this.getTotalCount());
		rs.close();

		return infos;
	}

	private Apppay102_Info wrapToModel(ResultSet rs) throws SQLException
	{
		...
		info.setORDER_NO(rs.getString("ORDER_NO"));
	...
	@Override
	public Apppay102_Info[] select(Apppay102_Info data) throws Exception
	{
		return null;
	}

	@Override
	public void update(Apppay102_Info data) throws Exception
	{
		...
		case AppayStatus.ALLOW:
			...
			Apppay102_Info info = this.findById(data.getAPPPAY_ID());
			// 账户账号
		...
		sql.append(" where APPPAY_ID=").append(data.getAPPPAY_ID());
		super.executeUpdate(sql.toString());
		super.debug("Apppay102--update-->" + sql.toString());
	...
	public void delete(int id) throws Exception
	{
		if (super.loadAppayStatus(TABLE, id) < 2)
		{
			...
			String delApp01 = "delete from APPPAY_102 where APPPAY_ID=" + id;
			...
			try
			{
				...
				super.debug("------->Apppay102删除<---------");
				super.debug("删除中间表个数--》" + temp);
				super.debug("------->Apppay102删除成功<---------");
			} catch (Exception e)
			{
				if (null != dba)
					dba.rollback();
	...
	{
		Apppay102_Info[] old = this.select("select * from " + TABLE
	...
	@Override
	public void operate(Apppay102_Info info) throws Exception
	{
		// 使用bind绑定信息
		super.initUpdate(TABLE);
		if (null != info.getAPPR_MAN())
			super.wrapUpdateSql("APPR_MAN", info.getAPPR_MAN());
		if (null != info.getORDER_NO())
			super.wrapUpdateSql("ORDER_NO", info.getORDER_NO());
	...
	@Override
	public Apppay102_Info findById(int id) throws Exception
	{
		String sql = "select * from " + TABLE + " where APPPAY_ID=" + id;
		ResultSet rs = super.execQuery(sql);
		Apppay102_Info info = null;
		if (rs.next())
		{
