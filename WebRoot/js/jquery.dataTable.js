...
// specify below
	...
	$('#cust_Table tr').each(function(i,row) {
...
//		$("#cust_Table tr:eq("+i+") :checkbox").each(function(j,checkbox) {
//			alert(this.value());
...
//		});
	...
	$('#merchant_Table tr').each(function(i,row) {
		$(row).removeAttr("class");
		if(i % 2==0){
			$(row).addClass('odd_row');
		}else{
		...
		$(row).click(function() {
		// toggle the checked state
		$("#merchant_Table tr:eq("+i+") :checkbox").each(function(j,checkbox) {
			if(applicableCheckbox(checkbox)) {
				// toggle checkbox states
				//alert(checkbox.checked);
				if (!checkbox.checked) {
	...
	$('#teminal_Table tr').each(function(i,row) {
		$(row).removeAttr("class");
		if(i % 2==0){
			$(row).addClass('odd_row');
		}else{
		...
		$(row).click(function() {
		// toggle the checked state
		$("#teminal_Table tr:eq("+i+") :checkbox").each(function(j,checkbox) {
			if(applicableCheckbox(checkbox)) {
				// toggle checkbox states
				//alert(checkbox.checked);
				if (!checkbox.checked) {
