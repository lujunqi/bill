...
function showLog(id){
	window.open ('../show/log.jsp?type=101&id='+id, 'log', 'height=600, width=1000, top=40, left=100,menubar=no, scrollbars=yes, resizable=yes,location=n o, status=no'); //这句要写成一行
}
...
function bussiness101Operate(id,index){
	...
	$.ajax( {
		...
		data : {'APP_TYPE':'101','APPPAY_ID':APPPAY_ID,'TEMINAL_INFO_ID':TEMINAL_INFO_ID,'TERM_ID1':TERM_ID1,'TERM_ID2':TERM_ID2,"COMMERCIAL_ID":${param.CommercialId}},
		type : 'post',
		success : function(id) {
			if(id<0){
				//alert("操作失败！");
		...
		if(request.getParameter("show")!=null){
			%>
						...
						$("#bussiness101form button[name='windowBackBtn']").removeAttr("disabled");
							$.post("../action/findById/Apppay101.jsp",{'id':${param.appayId}},function(json){
								var result=eval("("+json+")");
								//var result=json.resultset[0];
								//alert(result["COMMERCIAL_ID"]);
	...
	utils_submit = function(form, url) {
	 ...
	 function showAttachFile(id){
			window.open ('../insert/attachPhoto.jsp?type=101&id='+id, 'showAttach101', 'height=600, width=1000, top=40, left=100,menubar=no, scrollbars=yes, resizable=yes,location=n o, status=no'); //这句要写成一行 
		}
	 function showAppendPage(id){
			window.open ('AppendRemark.jsp?type=101&id='+id, 'showAppend101', 'height=150, width=300, top=40, left=100,menubar=no, scrollbars=0, resizable=0,location=n o, status=no'); //这句要写成一行 
		}
	function beforeSubmit(form) {
		var allow = "";
		$("#" + form + " input[type!='hidden'][type!='file'][class!='notRequired'],#"+form+" select").each(function(index, data) {
		...
		if ("" != allow) {
			...
			<%if(request.getParameter("show")==null){
			%>
			utils_submit(form,"../action/Apppay101DataInsert.jsp");
			<%}
			else{
				%>
				utils_submit(form,"../action/Apppay101DataUpdate.jsp");
				<%
			}
			%>
			//form.submit();
	        ...
	        for(i=(json.rows-1);i>=0;i--){
	        	...
	        	for(var pro in object[i]) {
	        		...
	        		if(object[i]["TEMINAL_STATUS"]==3)//审批通过
	        		{
	        			...
	        			row.find("#dosmthing").html("<button type='button' disabled='disabled' name='ajaxSubmitBtn' id='upBtn"+i+"' onclick='bussiness101Operate("+object[i]["TEMINAL_INFO_ID"]+","+i+")'>完成</button>");
		        		row.find("#terminal_number").html("<input  maxlength=8  value='"+ter2+"' disabled='disabled' name='terminal_number' id='terminal_number"+i+"'/>");
		        		row.find("#commercial_id").html("<input   maxlength=15   value='"+ter1+"' disabled='disabled' name='commercial_id' id='commercial_id"+i+"'/>");
	        		}
...
var ppppp=1;
function showAttachFileWindow(name){
	ppppp++;
  ...
  <div id="easyTabContext"  class="panel-container">
   		<div  class="left02">
			<div class="left02topDiv">
				...
				<div class="left02top_c" style="width: 1000px">
				<span id="titleName" >（实名支付）基本信息--客户单位名称:<font color="black" id="unitName"></font> </span>
				 </div>
			</div>
		</div>
		<div class="mycss">
		<form class="panel-container"  method="post"  action="#" id="bussiness101form">
			...
			<tr>
				...
				<td>
					<span>实名支付</span>
					<input type="hidden" name="APPTYPE" id="APPTYPE" value="101" readonly="readonly"/>
			...
			<tr>
				...
				<td>
					<input type="text" name='ACCOUNT_NO_CHECK' id="ACCOUNT_NO_CHECK"  onpaste="return false"/>
					<font>*</font>
				</td>
			</tr>
			<tr>	
			...
			<tr>
				...
				<td align="right">终端月保底:</td>
				<td>
					<input type="text" name="TER_MONTH_MIN" id="TER_MONTH_MIN" class="notRequired"/>
				</td>
			</tr>
			<tr>
			...
			<tr>
			<td align="right">实际经营范围：</td>
				<td>
					<input class="notRequired" id="REAL_OPERATE_RANGE" name="REAL_OPERATE_RANGE"/>
				</td>
   	...
   	<c:if test="${param.show==1 }">
		...
		<form action="../action/AppayApprove.jsp" id="bussinessOperationForm" method="post">
			<table>
			<input type="hidden" name="type" value='101' readonly="readonly"/>
		 ...
		 -->
			<tr>
				...
				<td>
					...
					<input id="APPR_REMARK" style="width: 300px" disabled="disabled" name="APPR_REMARK" required="false" style="width:100px"/>
				</td>
			</tr>
			<tr>
				<td>历史驳回信息：</td>
