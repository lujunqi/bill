...
function showLog(id){
	window.open ('../show/log.jsp?type=102&id='+id, 'log', 'height=600, width=1000, top=40, left=100,menubar=no, scrollbars=yes, resizable=yes,location=n o, status=no'); //这句要写成一行
}
...
function getCode(id){
	var id_temp = pad(id,8);
	return '01'+ addr.substring(2)+'0001'+ id_temp;
}
function find(cur_page,appStatus){
	...
	$.ajax({
		...
		dataType:'json',
		type:'POST',
		data:{"APPPAY_ID":<%=request.getParameter("appayId")==null?-1:request.getParameter("appayId")%>,apptype:102},
		success: function(json){
	        ...
	        for(i=(json.rows-1);i>=0;i--){
        		...
        		if(object[i]["TEMINAL_STATUS"]==3)//审批通过
        		{
        			...
        			var ter2=object[i]["TERM_ID2"];
        			if(null!=ter2 && $.trim(ter2) != ''){
        				ter2=$.trim(ter2);
        			}else{
        				ter2=getCode(object[i]["TEMINAL_INFO_ID"]);
        			}
	        		...
	        		row.find("#commercial_id").html("<input maxlength=8   value='"+ter1+"' disabled='disabeld' name='commercial_id' id='commercial_id"+i+"'/>");
        		}
	...
	function updateData3(){
	 ...
	 <script type="text/javascript">
		function bussiness101Operate(id,index){
			...
			var TERM_ID3=$("#TERM_ID1").val();
			var TERM_ID4=$("#TERM_ID2").val();
			...
			$.ajax( {
				...
				data : {'APP_TYPE':'102','APPPAY_ID':APPPAY_ID,'TEMINAL_INFO_ID':TEMINAL_INFO_ID,'TERM_ID1':TERM_ID1,'TERM_ID2':TERM_ID2,'TERM_ID3':TERM_ID3,'TERM_ID4':TERM_ID4,"COMMERCIAL_ID":${param.CommercialId}},
				type : 'post',
				success : function(id) {
					if(id<0){
						//alert("操作失败！");
		...
		%>
			$(function(){
					...
					$("#bussiness102form button[id='windowBackBtn']").removeAttr("disabled");
						$.post("../action/findById/Apppay102.jsp",{'id':${param.appayId}},function(json){
							...
							if (result["FEE_TYPE"] == 3){
								var fee = result["FEE_SET"];
								var i = fee.indexOf(',');
								if (i > 0){
									var fee_set = fee.substring(0,i);
									var fee_top = fee.substring((i+1),(i+3));
									$('#FEE_SET').val(fee_set);
									$('#FEE_SET_TOP').val(fee_top);
								}
							}
							$('#ORDER_NO').val(pad(${param.appayId},6));
...
function beforeSubmit(form,isFormSubmit) {
	...
	if ("" != allow) {
		...
		<%if(request.getParameter("show")==null){
		%>
		utils_submit(form,"../action/Apppay102DataInsert.jsp");
		<%}
		else{
			%>
			utils_submit(form,"../action/Apppay102DataUpdate.jsp");
			<%
		}
		%>
		//form.submit();
...
if($("#APPR_STATUS").val()==2)
	...
	if ("" != allow) {
		...
		if($('#TERM_ID1').val() == null || $('#TERM_ID1').val() == ''){
			$('#TERM_ID1').val(' ');
		}
		if($('#TERM_ID2').val() == null || $('#TERM_ID2').val() == ''){
			$('#TERM_ID2').val(' ');
		}		
		if(isFormSubmit){
			var f=document.getElementById(form);
...
function showAttachFile(id){
	window.open ('../insert/attachPhoto.jsp?type=102&id='+id, 'showAttach102', 'height=600, width=1000, top=40, left=100,menubar=no, scrollbars=yes, resizable=yes,location=n o, status=no'); //这句要写成一行 
}
function showAppendPage(id){
	window.open ('AppendRemark.jsp?type=102&id='+id, 'showAppend102', 'height=150, width=300, top=40, left=100,menubar=no, scrollbars=0, resizable=0,location=n o, status=no'); //这句要写成一行 
}
</script>
<style type="text/css">
  ...
  <div  class="panel-container">
   		<div  class="left02">
			<div class="left02topDiv">
				...
				<div class="left02top_c"  style="width: 1000px"  ><span id="titleName">（收付宝）基本信息---<font color="black" id="unitName"></font></span> </div>
			</div>
		</div>
		<form action="#" method="post"  id="bussiness102form">
			...
			<tr>
				...
				<td>
					<span>收付宝</span>
					<input type="hidden" name="APPTYPE" id="APPTYPE" value="102" readonly="readonly"/>
				...
				<td align='right'>手续费收取方向:</td>
				<td>
					<select name="ACCOUNT_TYPE" id="ACCOUNT_TYPE">
						<option value="">请选择...</option>
						<option value="2">向上游收取</option>
						<option value="1">向下游收取</option>
					</select>
					<font>*</font>
				</td>
			</tr>
			...
			<tr>
				<td align="right">账单信息:</td>
				<td>
					<input type="text" name="BILL_INFO" id="BILL_INFO"/>
					<font>*</font>
				</td>
		...
		<form action="../action/AppayApprove.jsp" id="bussiness102OperationForm" method="post" >
			...
			<tr>
				...
				<td>
					...
					<font>*</font>
					<input type="hidden" name="type" value='102' readonly="readonly"/>
