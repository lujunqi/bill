<%@page import="com.dodou.inferface.Export"%><%@ page contentType="text/html;charset=utf-8" language="java"%><%@ include file="../../../import.jsp"%><%
	...
	response.setHeader("Content-disposition",
			"attachment; filename=mchnt_" + date + "_dqd.txt");
	String sql = "select (select temp.term_id3 from term_app_table temp  where temp.apppay_id=a.apppay_id and temp.status = 1  and temp.apptype=105 group by temp.term_id3)as term_id3, memo, MARCHANT_NAME, MARCHANT_NAME2, Unit_area, account_bank, mcc18, marchanttype, mcc, Unit_area, bank_name, qingsuanno, lq_rule, account_no, account_name, comm_no, fee_rule1, fee_rule2, fee_content, fee_lev from Apppay_105 a, commercial_info c,ws_area w,bank_info b where appr_date = to_date('"
			+ date
			+ "', 'yyyy-MM-dd') and appr_status = 2 and a.commercial_id =c.commercial_id and w.areaid=c.unit_area and b.bank_code=a.account_bank and (fee_chan = 6 or fee_chan is null) and unit_no=4";
	...
	while (rs.next()) 
	{
		...
		//32域机构代码
		content.append("084821" + rs.getString("memo") + ",");
		//所属平台机构代码
		...
		content.append("1,");
		//受理地区代码
		content.append(rs.getString("memo") + ",");
