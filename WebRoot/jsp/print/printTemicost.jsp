...
<%
    ...
    String sql = "select (select count(*)           from temicost_his h          where h.teminal_info_id = i.teminal_info_id) as total,        i.teminal_info_id as seq,        (select '收单:' || t.term_id1           from term_app_table t          where t.teminal_info_id = i.teminal_info_id            and t.status = 1            and t.apptype = '105') ||        (select '实名支付:' || t.term_id1           from term_app_table t          where t.teminal_info_id = i.teminal_info_id            and t.status = 1            and t.apptype = '101') ||        (select '便捷付:' || t.term_id1           from term_app_table t          where t.teminal_info_id = i.teminal_info_id            and t.status = 1            and t.apptype = '103') as term_id1,        i.serial1,        c.comm_name||'-'|| (select a.marchant_name from apppay_105 a,term_app_table temp where temp.apppay_id=a.apppay_id and temp.teminal_info_id=i.teminal_info_id and temp.apptype=105 and temp.status=1) as comm_name,        t.money,        (select o.operman_name           from operman_info o          where o.operman_id = i.inst_man) as skr   from teminal_info i,  commercial_info c, temicost t  where i.teminal_info_id = "+teminal_info_id+"    and i.teminal_info_id = t.teminal_info_id    and i.commercial_id = c.commercial_id    and t.cost_type = 0";
